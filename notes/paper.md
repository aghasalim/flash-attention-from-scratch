...
ridge anywhere in 20.08–40.55, and 31.51 is inside that band, so **this hardware
...
macOS wheel and there is no NVIDIA GPU here, so tasks 03 and 05–10 of the plan
...
1937–3793 GFLOP/s, and separate runs an hour apart returned medians of 3142.6 and
2963.5. Bandwidth is stable by comparison (93.5–96.5 GB/s), so essentially all the
...
| **tiling (chunked vs naive)** | N≤2048 | **0.56–0.59× — slower** | AI 29.47 vs 31.51: tiling moves intensity the *wrong* way |
...
run at **0.91–0.98×**, i.e. slightly slower, since masking is extra work for
identical traffic. The SDPA path, which classifies blocks and skips the
fully-hidden ones, runs at **1.69–2.02×**, approaching the theoretical 2× and
...
- **`sdpa_kernel` is a no-op on MPS.** Forcing a backend silently does nothing —
  `sdpa_backend_honored=False` in the CSV. The "sdpa" rows are whatever kernel the
